...
class Scope3D:
    def __init__(self, thread):
        ...
        while self.thread.value[0] == 0:
            pass

        if self.thread.value[0] < 4096:
            self.RANGE_LEFT = 0
            self.RANGE_RIGHT = 4096
            self.xmax = self.RANGE_RIGHT
            self.ymax = self.RANGE_RIGHT
            self.zmax = self.RANGE_RIGHT
        else:
            self.RANGE_LEFT = 7000
            self.RANGE_RIGHT = 20000
            self.xmax = self.RANGE_RIGHT-7000
            self.ymax = self.RANGE_RIGHT-7000
            self.zmax = self.RANGE_RIGHT-7000
        self.xmin = self.RANGE_LEFT
        self.ymin = self.RANGE_LEFT
        self.zmin = self.RANGE_LEFT
        self.ax3d.set_xlim3d([self.RANGE_LEFT, self.RANGE_RIGHT])
        self.ax3d.set_xlabel('X')

        self.ax3d.set_ylim3d([self.RANGE_LEFT, self.RANGE_RIGHT])
        self.ax3d.set_ylabel('Y')

        self.ax3d.set_zlim3d([self.RANGE_LEFT, self.RANGE_RIGHT])
