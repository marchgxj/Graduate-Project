...
uint16 zcheck = 0;
uint8 check_error = 0;
uint8 data_sending = 0;

uint8 environment_over_count = 0;
uint8 force_assign_once = 0;

uint16 parking_zvalue[PARKING_ZVALUE_LENGTH];
uint8  parking_zvalue_count = 0;
uint16 zaxis_variance = 0;
...
//}
    ...
    {
        if((abs(xave-x_middle_quene[0])>1000)||(abs(yave-y_middle_quene[0])>1000)||(abs(zave-z_middle_quene[0])>1000))
        {
            return 0;
        }
...
// Author:xiaoximi
//}
    if((abs(ADvalueX-x_middle_quene[0])>1000)||(abs(ADvalueY-y_middle_quene[0])>1000)||(abs(ADvalueZ-z_middle_quene[0])>1000))
    {
        return;
    }
...
uint8 leaveRecognition(uint8 dis_divisor,uint8 min_divisor, uint16 env_threshold)
{
    ...
    for(uint8 i=0;i<MIDDLE_QUENE_LENGTH;i++)
    {
        ...
        if(distance < (MagneticUnit.parked_distance)/dis_divisor)
        {
            toggle_reason = 2;
        ...
        if(minus < MagneticUnit.Z_parked_distance/min_divisor)
        {
            toggle_reason = 3;
    ...
    if(vsEnvironment(env_threshold))
    {
        toggle_reason = 4;
        return 1;
    }
...
void IdentifyCar()
{
    ...
    {
        if(((abs(MagneticUnit.ZValue - z_middle_quene[0])>1400)
            ||abs(MagneticUnit.XValue - x_middle_quene[0])>1200)
            &&(force_assign_once==0)
                )
        {
            force_assign_once = 1;
        ...
        if(((MagneticUnit.compatness > COM_THRESHOLD)&&(diameterbuf>1000)||force_assign_once)
...
void TotalJudge()
{
       ...
       (MagneticUnit.VarState==CAR))
    {
        ...
        if(vsEnvironment(NORMAL_ENV_THRESHOLD))
        {
            ...
            if(environment_over_count>40)  //2 s
            {
    ...
    {
        zaxis_variance = getparkingVariance();
        if(zaxis_variance > ZAXIS_VARIANCE_THRESHOLD)
        {
            if(leaveRecognition(2,4,1000))
            {
                leave_recognition_count++;
            }
            if(leave_recognition_count>3)
            {
                quickCalibrate(1);
                parking_time = 0;
                parking_stable_flag = 0;
        ...
        {
            if(leaveRecognition(3,6,700))
            {
                leave_recognition_count++;
            }
            if(leave_recognition_count>150)
            {
                quickCalibrate(1);
                parking_time = 0;
                parking_stable_flag = 0;
...
//}
    ...
    else
    {
            ...
            if((abs(ADvalueX - VALUE_BASIC)<500)&&(abs(ADvalueY - VALUE_BASIC)<500)&&abs((ADvalueZ - VALUE_BASIC)<500))
            {
                return 1;
            }
    return 0;
...
void parkingStableSet()
{
       ...
       (parking_stable_flag == 0))
    {
        ...
        for(i=0;i<PARKING_ZVALUE_LENGTH;i++)
        {
            parking_zvalue[i] = 0;
        }
        MagneticUnit.XValue_parked_stable = MagneticUnit.XValue;
...
uint16 getparkingVariance()
{
// FunctionName: getparkingVariance
//{
// Parameter:none
//
// Return:variance of zvalue
//
// Description:
//  get the variance of z axis when vehicle is leaving
//
// Created: 2016/3/24
//
// Author:xiaoximi
//}
    ...
    parking_zvalue[parking_zvalue_count++] = MagneticUnit.ZValue;
    if(parking_zvalue_count>=PARKING_ZVALUE_LENGTH)
    {
        parking_zvalue_count = 0;
    }
    if(parking_zvalue[PARKING_ZVALUE_LENGTH-1]!=0)
    {
        return getVariance(parking_zvalue,PARKING_ZVALUE_LENGTH);
    }
